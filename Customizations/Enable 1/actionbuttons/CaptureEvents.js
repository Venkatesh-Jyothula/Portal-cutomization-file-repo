var selectedF;
var map = $NWP.map.getOlMapObject();
var newWindow = null;
...
//Loop on all the available services and retrieve the job service
		...
		if(args.name=="addressWithStatus3"){
			var formPathName = '/AddressCommand/custom_form.html';
			var formTitle = 'Address with Status 3 TEST';
			var width = 800;
			var height = 600;
			openForm(formPathName, formTitle, width, height);
		}		
		
		if(args.name=="addressAttributeUpdate"){
			var formPathName = '/AddressCommand/form_AddrAttrUpdates.html';
			var formTitle = 'Address Attribute Update';
			var width = 1000;
			var height = 600;
			openForm(formPathName, formTitle, width, height);
		}
		
		if(args.name=="form_AddrNetworkEntry"){
			var formPathName = '/AddressCommand/form_AddrNetworkEntry.html';
			var formTitle = 'Address to Network Association';
			var width = 800;
			var height = 600;
			openForm(formPathName, formTitle, width, height);
		}
...
function openForm(formPathName, formTitle, vwidth,vheight) 
{
	if (newWindow)
	{
        ...
        newWindow = $NWP.createWindow(
        {
            ...
            newWindow.show();  
	// Store the reference in the window object
