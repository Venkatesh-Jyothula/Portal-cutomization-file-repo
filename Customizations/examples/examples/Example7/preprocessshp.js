 ...
 *
 *  Adapted by JF Allard (2020):
 *   - all ESRI classes are now processed (in 2D)
 *   - Adapted SHPParser and DBFParser to include the filename currently processed
 *
 */
...
SHPParser.load = function(url, callback, returnData,fileName,zip,inputData) {

//console.log("ds shapeparser.load");
...
//console.log("Data="+geojsonData);
        callback(geojsonData['shp'], returnData,fileName,zip,inputData);
...
//console.log("length=" + length);
    ...
    case 28: // MultiPointM
		c = shape.content = {
            ...
            maxX: dv.getFloat64(idx+16, true),
            maxY: dv.getFloat64(idx+24, true),
            points: new Float64Array(dv.getInt32(idx+32, true)*2) //ignore the Z value (Z values are the end of the file)
        };
        idx += 36;
        for (i=0; i<c.points.length; i++) {
            c.points[i] = dv.getFloat64(idx, true);
...
if(i>0 && (i+1)%3==0)
{
	console.log("X="+c.points[i-2]+",Y="+c.points[i-1]);//+",Z="+c.points[i]);
}
*/
            idx += 8;
        }
    ...
    case 23: // PolylineM
    case 25: // PolygonM
//console.log("minX:"+dv.getFloat64(idx, true) + " ,minY="+dv.getFloat64(idx+8, true));
//console.log("maxX:"+dv.getFloat64(idx+16, true) + "maxY="+dv.getFloat64(idx+24, true));
		...
		c = shape.content = {
            ...
            maxY: dv.getFloat64(idx+24, true),
            parts: new Int32Array(dv.getInt32(idx+32, true)),
            points: new Float64Array(dv.getInt32(idx+36, true)*2) //ignore the Z value (Z values are the end of the file)
        };
        idx += 40;
        for (i=0; i<c.parts.length; i++) {
            c.parts[i] = dv.getInt32(idx, true);
...
DBFParser.load = function(url, encoding, callback, returnData,fileName,inputData) {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', url);
    xhr.responseType = 'arraybuffer';
    ...
    xhr.onload = function() {
        ...
        xhrText.onload = function() {
            ...
            callback(geojsonData['dbf'], returnData,fileName,inputData);
            URL.revokeObjectURL(url);
        };
